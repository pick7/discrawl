...
func TestHelpAndVersion(t *testing.T) {
	...
	require.Contains(t, out.String(), "0.11.1")
...
func TestShareCommandsPublishSubscribeAndUpdate(t *testing.T) {
	...
	err = Run(ctx, []string{"--config", cfgPath, "publish", "--tag", "invalid", "--no-commit"}, &bytes.Buffer{}, &bytes.Buffer{})
	require.Equal(t, 2, ExitCode(err))
	require.ErrorContains(t, err, "publish --tag requires a commit")

	out.Reset()
	require.NoError(t, Run(ctx, []string{"--config", cfgPath, "publish", "--tag", "test-snapshot", "--push"}, &out, &bytes.Buffer{}))
